---
name: daily-report
description: Jira / Confluence / MS Teams / Outlook 정보를 모아 일일 업무 보고 리포트를 생성할 때 사용. 사용자가 "오늘 일일보고", "데일리 리포트", "일일 업무 정리" 등을 요청하면 트리거.
argument-hint: [날짜 또는 "today"|"yesterday"]
---

# Daily Report

여러 협업 도구의 오늘자 활동을 한 줄 요약으로 정리해 화면에 출력한다. 사용자가 직접 복사해 원하는 채널에 붙여넣는다.

## When to use

- 사용자가 일일보고 / 데일리 리포트 / 업무 일지를 요청할 때
- "오늘 한 일 정리해줘", "/daily-report" 등으로 직접 호출
...
- 받은 메일: 오늘 수신, 미읽음 우선, 상위 25건 (자동 알림이 많아 limit을 넉넉히)
- 보낸 메일: 오늘 발신, 상위 10건 (요약은 제목만). 한국어 환경에서 폴더명 검색 실패 시 `sender = 본인 이메일` 필터로 폴백.

##### 자동 알림 분류 규칙

수집된 메일을 본문에 그대로 나열하지 말고, 다음 규칙으로 **"주요 메일"** 과 **"자동 알림"** 두 그룹으로 나눈다:

자동 알림으로 분류 (본문에는 한 줄 카운트만 노출):
- `sender == "noreply@telechips.com"` → 그룹: **Wiki 자동 알림**
  - 제목 prefix별로 sub-카운트: `[Wiki]`, `Recommended in WIKI`, 그 외
  - 가능하면 어떤 페이지/스페이스인지 상위 3개 키워드만 묶어서 한 줄(예: "D9_SWE_BSP, SeonENS Dolphin9, RND Planning 등")
- `sender` 도메인이 `microsoft.com`, `microsoft-noreply@*`, `no-reply-powerbi@*`, `*@e.adobe.com` 등 광고/구독 알림 → **시스템/광고 알림**
- 제목에 `(광고)` 포함 → **광고**
- `sender == "admin@hyundaicard.com"` 같은 정기 청구/명세 메일 → **정기 통지**
- IEEE/학회 CFP(`enotice@enotice.ieee.org` 등)는 마감 일자가 있으면 주요 메일에 유지, 없으면 자동 알림으로 강등

주요 메일 (본문에 개별 라인으로 노출):
- 위 자동 알림에 해당하지 않는 모든 외부 발신자
- 사람 이름이 보이는 발신자(개인 도메인 또는 회사 동료)
- 마감일/일정/요청이 포함된 메일

본문 출력 시:
- "주요" 섹션은 최대 7건. 7건 초과 시 우선순위 = (사람 발신자 > 마감/일정 포함 > 미읽음 > 첨부 있음) 순.
- "자동 알림" 섹션은 그룹별 카운트와 대표 키워드 1줄씩, 총 3줄 이하.
- Jira "Your Tasks for This Week" 같은 Jira 알림은 자동 알림 그룹 내 별도 한 줄로 표기.
...
- 헤더는 한국어 이모지 없이 텍스트로. 사용자가 그대로 복사해 다른 채널(메일/메신저 등)에 붙여넣을 수 있도록 plain text 유지.
...
- 본문을 사용자에게 코드블록 없이 그대로 출력한다. 사용자가 직접 복사해 원하는 채널에 붙여넣는다.
- 마지막에 한 줄로 "수집 완료. 총 N건" 정도의 짧은 메타 정보(전체 항목 수, 수집 실패한 소스)를 덧붙인다.

## 원칙

- **수집 실패 ≠ 전체 실패**: 일부 MCP 도구가 인증 만료 등으로 실패해도 다른 소스는 그대로 진행. 실패 사유는 "■ 메모" 섹션에 한 줄.
- **개인정보 최소화**: 화면 출력용이지만 메일/Teams 본문 전체를 그대로 옮기지 말고 제목·발신자·1줄 발췌만 포함.
